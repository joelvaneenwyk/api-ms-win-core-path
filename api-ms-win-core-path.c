...
HRESULT DLLAPI PathAllocCanonicalize(const WCHAR* path_in,
                                     DWORD flags,
                                     WCHAR** path_out);
HRESULT DLLAPI PathAllocCombine(const WCHAR* path1,
                                const WCHAR* path2,
                                DWORD flags,
                                WCHAR** out);
HRESULT DLLAPI PathCchAddBackslash(WCHAR* path, SIZE_T size);
HRESULT DLLAPI PathCchAddBackslashEx(WCHAR* path,
                                     SIZE_T size,
                                     WCHAR** endptr,
                                     SIZE_T* remaining);
HRESULT DLLAPI PathCchAddExtension(WCHAR* path,
                                   SIZE_T size,
                                   const WCHAR* extension);
HRESULT DLLAPI PathCchAppend(WCHAR* path1, SIZE_T size, const WCHAR* path2);
HRESULT DLLAPI PathCchAppendEx(WCHAR* path1,
                               SIZE_T size,
                               const WCHAR* path2,
                               DWORD flags);
HRESULT DLLAPI PathCchCanonicalize(WCHAR* out, SIZE_T size, const WCHAR* in);
HRESULT DLLAPI PathCchCanonicalizeEx(WCHAR* out,
                                     SIZE_T size,
                                     const WCHAR* in,
                                     DWORD flags);
HRESULT DLLAPI PathCchCombine(WCHAR* out,
                              SIZE_T size,
                              const WCHAR* path1,
                              const WCHAR* path2);
HRESULT DLLAPI PathCchCombineEx(WCHAR* out,
                                SIZE_T size,
                                const WCHAR* path1,
                                const WCHAR* path2,
                                DWORD flags);
HRESULT DLLAPI PathCchFindExtension(const WCHAR* path,
                                    SIZE_T size,
                                    const WCHAR** extension);
BOOL DLLAPI PathCchIsRoot(const WCHAR* path);
HRESULT DLLAPI PathCchRemoveBackslash(WCHAR* path, SIZE_T path_size);
HRESULT DLLAPI PathCchRemoveBackslashEx(WCHAR* path,
                                        SIZE_T path_size,
                                        WCHAR** path_end,
                                        SIZE_T* free_size);
HRESULT DLLAPI PathCchRemoveExtension(WCHAR* path, SIZE_T size);
HRESULT DLLAPI PathCchRemoveFileSpec(WCHAR* path, SIZE_T size);
HRESULT DLLAPI PathCchRenameExtension(WCHAR* path,
                                      SIZE_T size,
                                      const WCHAR* extension);
...
/* Find the last character of the root in a path, if there is one, without any
 * segments */
...
HRESULT DLLAPI PathAllocCanonicalize(const WCHAR* path_in,
  ...
  if (!path_in || !path_out ||
      ((flags & PATHCCH_FORCE_ENABLE_LONG_NAME_PROCESS) &&
       (flags & PATHCCH_FORCE_DISABLE_LONG_NAME_PROCESS)) ||
      (flags & (PATHCCH_FORCE_ENABLE_LONG_NAME_PROCESS |
                PATHCCH_FORCE_DISABLE_LONG_NAME_PROCESS) &&
       !(flags & PATHCCH_ALLOW_LONG_PATHS)) ||
      ((flags & PATHCCH_ENSURE_IS_EXTENDED_LENGTH_PATH) &&
       (flags & PATHCCH_ALLOW_LONG_PATHS))) {
    if (path_out)
      *path_out = NULL;
    return E_INVALIDARG;
  ...
  /* PATHCCH_ENSURE_IS_EXTENDED_LENGTH_PATH implies
   * PATHCCH_DO_NOT_NORMALIZE_SEGMENTS */
  if (flags & PATHCCH_ENSURE_IS_EXTENDED_LENGTH_PATH)
    flags |= PATHCCH_DO_NOT_NORMALIZE_SEGMENTS;
  ...
  if (root_end)
    root_end = buffer + (root_end - path_in);
...
HRESULT DLLAPI PathAllocCombine(const WCHAR* path1,
  ...
  if (!path1 || !path2)
    return PathAllocCanonicalize(path1 ? path1 : path2, flags, out);
  ...
  combined_path =
      HeapAlloc(GetProcessHeap(), 0, combined_length * sizeof(WCHAR));
  if (!combined_path) {
    *out = NULL;
    return E_OUTOFMEMORY;
  }
...
HRESULT DLLAPI PathCchAddExtension(WCHAR* path,
  ...
  if (!path || !size || size > PATHCCH_MAX_CCH || !extension)
    return E_INVALIDARG;

  next = extension;
  while (*next) {
    if ((*next == '.' && next > extension) || *next == ' ' || *next == '\\')
      return E_INVALIDARG;
    next++;
  }
  ...
  if (path_length + dot_length + extension_length + 1 > size)
    return STRSAFE_E_INSUFFICIENT_BUFFER;

  /* If extension is empty or only dot, return S_OK with path unchanged */
  if (!extension[0] || (extension[0] == '.' && !extension[1]))
    return S_OK;
...
HRESULT DLLAPI PathCchAppendEx(WCHAR* path1,
  ...
  /* Create a temporary buffer for result because we need to keep path1
   * unchanged if error occurs. And PathCchCombineEx writes empty result if
   * there is error so we can't just use path1 as output buffer for
   * PathCchCombineEx */
  ...
  if (path2 && path2[0] == '\\' && path2[1] != '\\')
    path2++;

  hr = PathCchCombineEx(result, size, path1, path2, flags);
  if (SUCCEEDED(hr))
    memcpy(path1, result, size * sizeof(WCHAR));
...
HRESULT DLLAPI PathCchCanonicalize(WCHAR* out, SIZE_T size, const WCHAR* in) {
  /* Not X:\ and path > MAX_PATH - 4, return
...
HRESULT DLLAPI PathCchCanonicalizeEx(WCHAR* out,
  ...
  length = lstrlenW(buffer);
  if (size < length + 1) {
    /* No root and path > MAX_PATH - 4, return
     * HRESULT_FROM_WIN32(ERROR_FILENAME_EXCED_RANGE) */
    if (length > MAX_PATH - 4 &&
        !(in[0] == '\\' || (is_drive_spec(in) && in[2] == '\\')))
...
HRESULT DLLAPI PathCchCombineEx(WCHAR* out,
  ...
  if (!out || !size || size > PATHCCH_MAX_CCH)
    return E_INVALIDARG;
...
BOOL DLLAPI PathCchIsRoot(const WCHAR* path) {
  ...
  if (is_unc || (path[0] == '\\' && path[1] == '\\' && path[2] != '?')) {
    ...
    if (get_next_segment(next, &next) && !*next)
      return FALSE;
...
HRESULT DLLAPI PathCchRemoveBackslashEx(WCHAR* path,
  ...
  if (path_length == path_size && !path[path_length])
    return E_INVALIDARG;
...
HRESULT DLLAPI PathCchRemoveFileSpec(WCHAR* path, SIZE_T size) {
  ...
  /* The backslash at the end of UNC and \\* are not considered part of root in
   * this case */
...
HRESULT DLLAPI PathCchStripToRoot(WCHAR* path, SIZE_T size) {
  ...
  if (!path || !*path || !size || size > PATHCCH_MAX_CCH)
    return E_INVALIDARG;
